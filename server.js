...
const dbConfig = {
  ...
  database: 'ContractManager',       // Your database name
  options: {
    keepAlive: true,
...
app.post('/api/updateTest', async (req, res) => {
  ...
  try {
    const { id, text } = req.body;
      ...
      // Execute the SQL query
      //const testquery = await pool.request().query(`UPDATE DadosCláusulas SET texto = 'texto' WHERE id = '1'`); 
...
async function getClauses() {
  try {
    ...
    // Execute the SQL query
    const result = await pool.request().query('SELECT * FROM DadosCláusulas');
...
app.get('/api/clauses', async (req, res) => {
  try {
    ...
    res.json(clauses);  // Return the rows to the client
  } catch (error) {
    console.error('SQL Query Error:', error);
    res.status(500).send("Error querying database");
  }
...
app.post('/api/updateClauseText', async (req, res) => {
  ...
  try {
    ...
    console.log("id: "+id+" texto: "+texto);
      ...
      // Execute the SQL query
      //const testquery = await pool.request().query(`UPDATE DadosCláusulas SET texto = 'texto' WHERE id = '1'`); 
      const result = await pool.request().query(`UPDATE DadosCláusulas SET texto = '`+texto+`' WHERE id = '`+id+`'`);
...
app.post('/api/updateClauseTextOld', async (req, res) => {
  ...
  try {
    ...
    const query = `UPDATE DadosCláusulas SET texto = @texto WHERE id = @id`;
